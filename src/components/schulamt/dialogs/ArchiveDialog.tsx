    ...
    <Dialog open={!!archiveTeacher} onOpenChange={(open) => !open && setArchiveTeacher(null)}>
      <DialogContent className="max-w-2xl sm:max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 py-4">
          {archiveData.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">Keine bisherigen Einsätze gefunden.</div>
          ) : (
            <div className="space-y-3">
              {archiveData.map((assignment: AssignmentData) => (
                <div key={assignment.id} className="p-4 border border-border rounded-xl bg-muted/50">
                    <div className="font-bold mb-1">{assignment.request?.school.name}</div>
                    <div className="text-xs text-muted-foreground mb-2">{assignment.request?.schedule ? 'Individueller Plan' : `ab ${assignment.request?.startHour}. Stunde (${assignment.request?.hours}h)`}</div>
